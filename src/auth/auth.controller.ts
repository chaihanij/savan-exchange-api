...
import { RefreshTokenDto, SignInDto, SignInResponseDto, SignUpDto } from './dtos';
...
export class AuthController {
  ...
  @Post('signin')
  @HttpCode(HttpStatus.OK)
  async signIn(@Body() signInDto: SignInDto, @Res({ passthrough: true }) res: Response): Promise<SignInResponseDto> {
  ...
  async refresh(@Body() refreshTokenDto: RefreshTokenDto, @Res({ passthrough: true }) res: Response) {
    const { accessToken, refreshToken, account } = await this.authService.refreshToken(refreshTokenDto);
    this.setCookies(res, accessToken, refreshToken);
    return Object.assign({ accessToken, refreshToken }, account);
  }
